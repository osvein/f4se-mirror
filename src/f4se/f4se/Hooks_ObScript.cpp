...
RelocAddr <uintptr_t> GetActorBaseHeadData_Start(0x00000000 + 0xCF);
RelocAddr <uintptr_t> GetMorphGroups_Start(0x00000000 + 0x390);
RelocAddr <uintptr_t> StaticTextureIndexed_Ctor_Start(0x00000000 + 0xD38);
...
bool F4SETestCode_Execute(void * paramInfo, void * scriptData, TESObjectREFR * thisObj, void * containingObj, void * scriptObj, void * locals, double * result, void * opcodeOffsetPtr)
{
	...
	if(thisObj) {
		...
		if(actor)
		{
			auto vm = (*g_gameVM)->m_virtualMachine;
			IObjectHandlePolicy	* policy = vm->GetHandlePolicy();
			UInt64 handle = policy->Create(Actor::kTypeID, actor);

			vm->ForEachIdentifier(handle, [&vm](VMIdentifier * obj)
			{
				_MESSAGE("Type: %s", obj->m_typeInfo->m_typeName.c_str());
				if(obj->m_typeInfo->memberData.unk00 == 3)
				{
					...
					{
						const char * varName = obj->m_typeInfo->properties->defs[i].propertyName.c_str();

						VMValue val(obj->properties[i]);
						_MESSAGE("%s: Type: %d, Value: %016I64X", varName, val.GetTypeEnum(), val.data.p);
					}
					gLog.Outdent();
				}
			...
			class AliasVisitor : public IAliasFunctor
			{
			public:
				virtual UInt32 Visit(BGSRefAlias * alias) override
				{
					auto vm = (*g_gameVM)->m_virtualMachine;
					IObjectHandlePolicy	* policy = vm->GetHandlePolicy();
					UInt64 handle = policy->Create(BGSRefAlias::kTypeID, alias);
					vm->ForEachIdentifier(handle, [&vm](VMIdentifier * obj)
					{
						_MESSAGE("Type: %s", obj->m_typeInfo->m_typeName.c_str());
						if(obj->m_typeInfo->memberData.unk00 == 3)
			...
			actor->ForEachAlias(&visitor);

			/*ActorEquipData * equipData = actor->equipData;
			if(!equipData)
				return false;
			for(UInt32 iSlotIndex = 0; iSlotIndex < ActorEquipData::kMaxSlots; ++iSlotIndex)
			{
				_MESSAGE("SlotIndex: %d", iSlotIndex);

				// Make sure there is an item in this slot
				auto item = equipData->slots[iSlotIndex].model;
				if(item) {
					DumpClass(item, sizeof(TESObjectARMA)/8);
				}
	...
	if(0) {
		...
		if(toggle)
		{
			BGSMaterialSwap * matSwap1 = static_cast<BGSMaterialSwap *>(LookupFormByID(0x1159A7));
			if(extraDataList)
			{
				...
				if(extraData)
				{
					if(matSwap1)
					{
						...
						if(materialSwap)
						{
							materialSwap->materialSwap = matSwap1;
						}
					}
					else
					{
				...
				{
					extraDataList->Add(ExtraDataType::kExtraData_MaterialSwap, ExtraMaterialSwap::Create(matSwap1));
				}
			}
		}
		else
		{
			BGSMaterialSwap * matSwap2 = static_cast<BGSMaterialSwap *>(LookupFormByID(0x1159A4));
			if(extraDataList)
			{
				BSExtraData * extraData = extraDataList->GetByType(ExtraDataType::kExtraData_MaterialSwap);
				if(extraData)
				{
					if(matSwap2)
					{
						...
						if(materialSwap)
						{
							materialSwap->materialSwap = matSwap2;
						}
					}
					else
					{
				...
				{
					extraDataList->Add(ExtraDataType::kExtraData_MaterialSwap, ExtraMaterialSwap::Create(matSwap2));
				}
			}
		}
