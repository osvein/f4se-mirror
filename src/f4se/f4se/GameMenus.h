...
public:
	...
	DEFINE_MEMBER_FN(LoadCharacterParameters, void, 0x00B197F0); // This function updates all the internals from the current character
...
protected:
	MEMBER_FN_PREFIX(UI);
	DEFINE_MEMBER_FN(RegisterMenu, void, 0x01FD0770, const char * name, CreateFunc creator, UInt64 unk1);
	DEFINE_MEMBER_FN(IsMenuOpen, bool, 0x01FCEC00, BSFixedString * name);
